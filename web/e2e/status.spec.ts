...
test('renders a complete Nazhua homepage with one shell, map points and cycle-aware cards', async ({ page }) => {
  ...
  await expect(page.locator('.nazhua-card').first().locator('.nazhua-card__spec')).toContainText('2C/2G/20G')
...
test('desktop exposes the card and ServerStatus list modes', async ({ page }, testInfo) => {
  ...
  expect(tableMark.flagWidth).toBeGreaterThanOrEqual(22)
  expect(tableMark.flagWidth).toBeLessThanOrEqual(26)
  expect(tableMark.flagHeight).toBeGreaterThanOrEqual(14)
  expect(tableMark.flagHeight).toBeLessThanOrEqual(18)
  ...
  await expect(page.locator('.nazhua-status-table__head [role="columnheader"]')).toHaveCount(14)
