...
可靠探测、从端部署、探测型路由（须自备 nexttrace）、保留策略和升级顺序见 [可靠探测运维指南](docs/reliable-telemetry.md)。Telegram 机器人见 [bot.md](docs/bot.md)。本版本只接受全新数据库；若数据库非空且没有 `schema_migrations`，进程会拒绝启动并保留原文件供诊断。
